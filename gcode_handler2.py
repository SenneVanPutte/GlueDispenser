	...
	def wait_probe_stop(self, queue):
		...
		while not probe_stopped:
			...
			try:
				...
				resp_dict = {}
			if self.prb_catch:
				queue.put(self.prb_pos)
				self.log_file.write("exterior\n")
				self.prb_catch = False
				self.prb_pos = [-1, -1, -1]
				return
		...
		for l in bloc.splitlines():
			...
			while ((line) and not self.kill or not responded) and capture_response:
				...
				if "er" in resp_dict: 
					print("######--WARNING--######")
					print(l, line)
				elif "r" in resp_dict and "prb" in resp_dict["r"]:
					self.prb_pos = [resp_dict["r"]["prb"]["x"], resp_dict["r"]["prb"]["y"], resp_dict["r"]["prb"]["z"]]
					self.prb_catch = True
				if not line == unicode(""):
